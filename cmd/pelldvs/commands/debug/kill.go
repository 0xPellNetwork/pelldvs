...
var killCmd = &cobra.Command{
	...
	Short: "Kill a PellDVS process while aggregating and packaging debugging data",
	Long: `Kill a PellDVS process while also aggregating PellDVS process data
...
Example:
$ pelldvs debug 34255 /path/to/cmt-debug.zip`,
...
func killCmdHandler(_ *cobra.Command, args []string) error {
	...
	logger.Info("killing PellDVS process")
...
// killProc attempts to kill the PellDVS process with a given PID with an
...
func killProc(pid int, dir string) error {
	// pipe STDERR output from tailing the PellDVS process to a file
	//
	...
	// kill the underlying PellDVS process and subsequent tailing process
	go func() {
		// Killing the PellDVS process with the '-ABRT|-6' signal will result in
		// a goroutine stacktrace.
		p, err := os.FindProcess(pid)
		if err != nil {
			fmt.Fprintf(os.Stderr, "failed to find PID to kill PellDVS process: %s", err)
		} else if err = p.Signal(syscall.SIGABRT); err != nil {
			fmt.Fprintf(os.Stderr, "failed to kill PellDVS process: %s", err)
		}

		// allow some time to allow the PellDVS process to be killed
		//
		...
		if err := cmd.Process.Kill(); err != nil {
			fmt.Fprintf(os.Stderr, "failed to kill PellDVS process output redirection: %s", err)
		}
	}()
