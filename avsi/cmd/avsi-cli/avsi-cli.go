...
import (
	...
	"github.com/0xPellNetwork/pelldvs/avsi/version"
...
var RootCmd = &cobra.Command{
	Use:   "avsi-cli",
	Short: "the AVSI CLI tool wraps an AVSI client",
	Long:  "the AVSI CLI tool wraps an AVSI client and is used for testing AVSI servers",
	PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
		...
		if logger == nil {
			filter, err := log.ParseLogLevel(flagLogLevel)
			if err != nil {
				return err
			}
			logger = log.NewLogger(os.Stdout, log.FilterOption(filter))
		}
		if client == nil {
			var err error
			client, err = avsicli.NewClient(flagAddress, flagAbci, false)
			if err != nil {
				return err
			}
			client.SetLogger(logger.With("module", "avsi-client"))
...
func addGlobalFlags() {
	...
	RootCmd.PersistentFlags().StringVarP(&flagAbci, "avsi", "", "socket", "either socket or grpc")
...
func addCommands() {
	...
	RootCmd.AddCommand(processRequestCmd)
...
func muxOnCommands(cmd *cobra.Command, pArgs []string) error {
	...
	case "request":
		return processRequest(cmd, actualArgs)
	default:
...
var processRequestCmd = &cobra.Command{
	Use:   "request",
	Short: "dvs ProcessRequest",
...
var versionCmd = &cobra.Command{
	Use:   "version",
	Short: "print AVSI console version",
	Long:  "print AVSI console version",
	Args:  cobra.ExactArgs(0),
...
var kvstoreCmd = &cobra.Command{
	Use:   "kvstore",
	Short: "AVSI demo example",
	Long:  "AVSI demo example",
	Args:  cobra.ExactArgs(0),
	RunE:  cmdKVStore,
}
...
func processRequest(cmd *cobra.Command, args []string) error {
	if len(args) != 2 {
		return fmt.Errorf("require two params key and value")
	}

	data := []byte(args[0] + "=" + args[1])

	res, err := client.ProcessDVSRequest(cmd.Context(), &types.RequestProcessDVSRequest{Request: &types.DVSRequest{Data: data}})
	if err != nil {
		return err
	}
	printResponse(cmd, args, response{
...
func cmdKVStore(*cobra.Command, []string) error {
	...
	srv.SetLogger(logger.With("module", "avsi-server"))
