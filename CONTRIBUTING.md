...
Thank you for your interest in contributing to PellDVS! Before contributing, it
may be helpful to understand the goal of the project. The goal of PellDVS is to
develop a decentralized verifiable service robust enough to support permissionless
...
PellDVS project or a potential fork. When targeting the main PellDVS project,
the following process leads to the best chance of landing changes in `main`.

All work on the code base should be motivated by a [GitHub
Issue](https://github.com/0xPellNetwork/pelldvs/issues).
[Search](https://github.com/0xPellNetwork/pelldvs/issues?q=is%3Aopen+is%3Aissue+label%3A%22good+first+issue%22)
...
Issue](https://github.com/0xPellNetwork/pelldvs/issues/new/choose). The issue helps
...
Comment](https://github.com/0xPellNetwork/pelldvs/tree/main/docs/rfc) in the
PellDVS repo. Discussion at the RFC stage will build collective
...
While my fork lives at `https://github.com/0xPellNetwork/pelldvs,
the code should never exist at `$GOPATH/src/github.com/0xPellNetwork/pelldvs`.
Instead, we use `git remote` to add the fork as a new remote for the original repo,
`$GOPATH/src/github.com/0xPellNetwork/pelldvs`, and do all the work there.

For instance, to create a fork and work on a branch of it, I would:

- Create the fork on GitHub, using the fork button.
- Go to the original repo checked out locally (i.e. `$GOPATH/src/github.com/0xPellNetwork/pelldvs`)
- `git remote rename origin upstream`
- `git remote add origin git@github.com:ebuchman/basecoin.git`

Now `origin` refers to my fork and `upstream` refers to the PellDVS version.
So I can `git push -u origin main` to update my fork, and make pull requests to PellDVS from there.
...
That said, the `main` branch of every PellDVS repository should just build
...
- `issue or PR number` is the PellDVS issue number, if one exists, or the PR
  number, otherwise.
...
- [mempool] \#1112 Refactor Go API to be able to easily swap out the current
  mempool implementation in PellDVS forks
...
`github.com/0xPellNetwork/pelldvs`, or your fork (using `git remote add origin`).
...
finishes the sentence "This change modifies PellDVS to...". For example,
...
End-to-end tests are used to verify a fully integrated PellDVS network.
