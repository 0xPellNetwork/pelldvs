...
// clearConfig clears env vars, the given defaultRoot dir, and resets viper.
...
func testSetup(t *testing.T, root string, args []string, env map[string]string) error {
	...
	rootCmd := testRootCmd()
	cmd := cli.PrepareBaseCmd(rootCmd, "PELLDVS", root)
...
func TestRootHome(t *testing.T) {
	...
	cases := []struct {
		...
		{nil, nil, root, root},
		{[]string{"--home", newRoot}, nil, newRoot, newRoot},
		{nil, map[string]string{"PELLDVSHOME": newRoot}, "", newRoot},
		{nil, map[string]string{"PELLDVS_HOME": newRoot}, "", newRoot},
	}

	for i, tc := range cases {
		idxString := "idx: " + strconv.Itoa(i)
		err := testSetup(t, tc.defaultRoot, tc.args, tc.env)
		require.Nil(t, err, idxString)
		assert.Equal(t, tc.exceptRoot, config.RootDir, idxString)
		assert.Equal(t, tc.exceptRoot, config.P2P.RootDir, idxString)
	}
}
...
func TestRootFlagsEnv(t *testing.T) {
	...
	cases := []struct {
		...
		{nil, map[string]string{"PD_LOW": "debug"}, defaultLogLvl},           // wrong env flag
		{nil, map[string]string{"PD_LOG_LEVEL": "debug"}, defaultLogLvl},     // wrong env prefix
		{nil, map[string]string{"PD_LOG_LEVEL": "debug"}, defaultLogLvl},     // right, but deprecated env
		{nil, map[string]string{"PELLDVS_LOW": "debug"}, defaultLogLvl},      // wrong env flag
		{nil, map[string]string{"PELLDVS_LOG_LEVEL": "info"}, defaultLogLvl}, // wrong env prefix
		{nil, map[string]string{"PELLDVS_LOG_LEVEL": "debug"}, "debug"},      // right env
	}
...
func TestRootConfig(t *testing.T) {
	...
	cases := []struct {
		...
		{nil, map[string]string{"PELLDVS_LOG_LEVEL": "abc:info"}, "abc:info"}, // env over rides
	}
