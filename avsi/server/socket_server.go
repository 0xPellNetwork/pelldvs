...
func NewSocketServer(protoAddr string, app types.Application) service.Service {
	...
	s.BaseService = *service.NewBaseService(nil, "AVSIServer", s)
	return s
}
...
func (s *SocketServer) handleRequest(ctx context.Context, req *types.Request) (*types.Response, error) {
	...
	case *types.Request_ProcessDvsRequest:
		res, err := s.app.ProcessDVSRequest(ctx, r.ProcessDvsRequest)
		if err != nil {
			return nil, err
		}
		return types.ToResponseProcessRequest(res), nil

	case *types.Request_ProcessDvsResponse:
		res, err := s.app.ProcessDVSResponse(ctx, r.ProcessDvsResponse)
		if err != nil {
			return nil, err
		}
		return types.ToResponseProcessDVSResponse(res), nil
