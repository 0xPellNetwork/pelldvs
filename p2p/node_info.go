...
import (
	...
	cmtbytes "github.com/0xPellNetwork/pelldvs/libs/bytes"
	cmtstrings "github.com/0xPellNetwork/pelldvs/libs/strings"
	tmp2p "github.com/0xPellNetwork/pelldvs/proto/pelldvs/p2p"
...
// between two peers during the PellDVS P2P handshake.
...
type DefaultNodeInfoOther struct {
	DvsRequestIndex string `json:"dvs_request_index"`
...
func (info DefaultNodeInfo) Validate() error {
	...
	// Validate Other.
	other := info.Other
	dvsIndex := other.DvsRequestIndex
	switch dvsIndex {
	case "", "on", "off":
	default:
		return fmt.Errorf("info.Other.DvsIndex should be either 'on', 'off', or empty string, got '%v'", dvsIndex)
	}
...
func (info DefaultNodeInfo) ToProto() *tmp2p.DefaultNodeInfo {
	...
	dni.Other = tmp2p.DefaultNodeInfoOther{
		DvsRequestIndex: info.Other.DvsRequestIndex,
...
func DefaultNodeInfoFromToProto(pb *tmp2p.DefaultNodeInfo) (DefaultNodeInfo, error) {
	...
	dni := DefaultNodeInfo{
		...
		Other: DefaultNodeInfoOther{
			DvsRequestIndex: pb.Other.DvsRequestIndex,
