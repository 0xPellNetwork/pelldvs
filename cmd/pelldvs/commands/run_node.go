...
// These are exposed for convenience of commands embedding a PellDVS node
func AddNodeFlags(cmd *cobra.Command) {
	...
	cmd.Flags().String("avsi", config.AVSI, "specify avsi transport (socket | grpc)")
...
// It can be used with a custom PrivValidator and in-process AVSI application.
func NewRunNodeCmd(nodeProvider nm.Provider) *cobra.Command {
	cmd := &cobra.Command{
		Use:     "start",
		Aliases: []string{"node", "run"},
		Short:   "Run the PellDVS node",
		RunE: func(cmd *cobra.Command, args []string) error {
			if err := checkGenesisHash(config); err != nil {
				return err
			}

			cmtlogger := cmtlog.NewLogger(os.Stdout)
			n, err := nodeProvider(config, cmtlogger)
			if err != nil {
